...
{
    std::cout << "Parametric constructor ClapTrap called" << std::endl;
    return;
}
...
void ClapTrap::attack(std::string const &target)
{
    if (this->m_hit_points > 0)
    {
        if (this->m_energy_points > 0)
        {
            ...
            std::cout   << "ClapTrap " << this->m_name << " uses 1 point of energy to attack "
                        << target << ", causing " << this->m_attack_damage
                        << " points of damage ! He still has "
                        << this->m_energy_points << " energy points." << std::endl;
        }
        else
            std::cout   << "ClapTrap "<< this->m_name << " has no more energy points for attack someone." << std::endl;
    }
    else
        std::cout << "ClapTrap " << this->m_name << " can't attack, he is already dead." << std::endl;
...
void ClapTrap::takeDamage(unsigned int amount)
{
    if (this->m_hit_points > 0)
    {
        if (m_hit_points > amount)
        {
            ...
            std::cout << "ClapTrap " << this->m_name << " received " << amount << " damages."
...
{
    std::cout   << "ClapTrap " << this->m_name << " currently has " << this->m_hit_points << "/10 HP and "
                << this->m_energy_points << "/10 EP." << std::endl;
    if (this->m_hit_points == 0)
        std::cout << "ClapTrap "<< this->m_name << " has resurrected." << std::endl;
    else
        std::cout << "ClapTrap "<< this->m_name << " has been repaired." << std::endl;
    ...
    std::cout   << "ClapTrap " << this->m_name << " has after being repaired " << this->m_hit_points
